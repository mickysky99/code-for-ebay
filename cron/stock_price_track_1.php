...
$query = $connection->prepare("SELECT * FROM imported_products WHERE (id between '1' and '250000') and (processed = 'no') LIMIT 1000");
$query->execute();
...
if ($imported_products_total > 0 and empty($imported_products_not_processed))
{
   ...
   $query = $connection->prepare("UPDATE imported_products SET processed = 'no' WHERE (id between '1' and '250000')");
   $query->execute();
}
